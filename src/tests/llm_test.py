...
@pytest.mark.parametrize("model_name, quantization", params)
def test_hf_local_pipe(model_name, quantization):
    llm_ = LLM(quantization=quantization, model_name=model_name, pipeline='hf')
    model = llm_.load_model()
    response = model.invoke("Who are you?")
    assert isinstance(response, Text)
    del model
...
def test_hf_web_pipe(model_name, quantization):
    llm_ = LLM(quantization=quantization, model_name=model_name, pipeline='hf')
